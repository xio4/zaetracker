...
const showTimeEntry = (config = defaultProjectConfig, zWorklog) => {
    if (!zWorklog || !zWorklog.start) {
        showInfo('zaebutton is inactive', 'green');

        return;
    }

    const start = moment(zWorklog.start, moment.ISO_8601);
    let duration = zWorklog.duration || 0;
    ...
    showInfo(`${zWorklog.description} | ${start.format(TIME_ENTRY_DATE_FORMAT)} | ${start.format(TIME_ENTRY_FORMAT)}-${end.format(TIME_ENTRY_FORMAT)} | ${moment.utc(duration).format(DURATION_FORMAT)}`, 'green');
};
...
const getZaebuttonConfig = pathOr({}, ['zaebutton']);
...
const createPost = curryN(4, (url, headers, method, body) => 
    superagent.post(`${url}${method}`).send(body).set({ ...defaultHeaders, ...headers })
);
const createGet = curryN(3, (url, headers, method) => 
    superagent.get(`${url}${method}`).set({ ...defaultHeaders, ...headers })
); 
const createPut = curryN(4, (url, headers, method, body) => 
    superagent.put(`${url}${method}`).send(body).set({ ...defaultHeaders, ...headers })
); 
const wrapToPromise = fn => promise => Promise.resolve(promise).then(fn);
const getBody = wrapToPromise(prop('body'));
const getData = wrapToPromise(prop('data'));
